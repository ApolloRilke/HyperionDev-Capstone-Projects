...
# Isolate the usernames and the passwords separately.
sort_user_info = []
for line in user_file:
    sort_user_info.append(line.split(", "))

for i in range(0, len(sort_user_info)):
    usernames.append(sort_user_info[i][0])
    passwords.append(sort_user_info[i][1])
...
while x == 1:
    ...
    if passwords.count(password) > 0:
        login = f"{username}, {password}"
        if user_file.count(login) > 0:
            print("This should be a successful login. \n")
            x += 1
        else:
...
if option == "r":
    x = 2
    while x == 2:
        username_new = input("Enter a new username: \n")
        if username_new not in usernames:
            ...
            while password_new_1 != password_new_2:
                ...
                password_new_2 = input("Confirm the password: \n")
            x += 1
        elif username_new in usernames:
            print("Username already exists.")
    print(f"{username_new}, {password_new_1}")
